...
## プロジェクト概要
AIベクトルUI生成ツール - JSONスキーマからSVG/PNG/PDFを生成し、UIエディターで画面レイアウトを組み立て、ゲームエンジンに統合するシステム

> **設計思想の全体像**: `docs/ARCHITECTURE_PIPELINE.md` を参照。
> AIが生成したPNG/SVGをElectronエディターのコンポーネントにアタッチし、
> 階層構造のレイアウトJSONを出力してAxmolエンジンに組み込む3レイヤー構成。
