...
## texture（テクスチャ・クリッピングマスク）
レイヤーに対して、生成AIなどで作成した画像ファイルをSVGクリッピングマスクで適用できます（アセンブラ機能）。

**仕様**
- 現在は `rectLayer` (`shape: "roundedRect"`) に対応しています。設定された角丸などを維持したまま画像を切り抜いて合成します。
- `fill`（下敷き色）と `stroke`（輪郭線）の間に画像レイヤーが挿入されます。
- `path` には、ローカルファイルの絶対パスやURLを指定します。

**例**
```json
"texture": {
  "path": "file:///path/to/generated_image.png"
}
```
